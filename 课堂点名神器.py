...
class APP:
    ...
    def create_widget(self):
        ...
        self.label_show_name=ttk.Label(self.root,textvariable=self.label_show_name_var,font=('Arial', 62,"bold"),foreground = '#1E90FF')
        self.btn_start=ttk.Button(self.root,text="开始",)
        self.btn_load_names=ttk.Button(self.root,text="导入学生名单",)
        ...
        self.label_show_name_num=ttk.Label(self.root,font=('Arial', 25),foreground = '#FF7F50')
    ...
    def set_widget(self):
        ...
        if init_names:
            ...
            self.label_show_name_num.config(text=f"请先导入学生名单！")
    ...
    def start_point_name(self):
        ...
        if len(self.default_names)==1:
            messagebox.showinfo("提示",'名单里就一个人，不用选了！')
    ...
    def load_names(self):
        ...
        if filename:
            ...
            if names:
                ...
                self.label_show_name_num.config(text=f"共导入{len(self.default_names)}个姓名")
                default_name_ = "幸运儿会是谁呢？"
                self.label_show_name_var.set(default_name_)
                self.label_show_name_adjust(default_name_)
